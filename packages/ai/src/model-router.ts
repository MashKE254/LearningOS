...
export const QueryComplexitySchema = z.object({
  tier: z.enum(['HAIKU', 'SONNET', 'OPUS', 'VERIFIED']),
  confidence: z.number().min(0).max(1),
  reasoning: z.string(),
  requiresVerification: z.boolean(),
  verificationType: z.enum(['MATH', 'CHEMISTRY', 'CODE', 'NONE']).optional(),
});
...
export class ModelRouter {
  ...
  private usageMetrics: Map<ModelTier, { tokens: number; calls: number }>;
   ...
   */
  async classifyQuery(query: string, context?: {
    sessionMode?: string;
    studentProfile?: {
  ...
  async route(
    ...
    const model = MODELS[tier as keyof typeof MODELS];
    ...
    // Update metrics
    const metrics = this.usageMetrics.get(tier as ModelTier)!;
   ...
   */
  async quickClassify(input: string, options: string[]): Promise<string> {
    const response = await this.anthropic.messages.create({
      model: MODELS.HAIKU,
      max_tokens: 100,
      temperature: 0,
   ...
   * Get usage metrics
   */
  getMetrics(): Record<ModelTier, { tokens: number; calls: number; estimatedCost: number }> {
    ...
    const result: Record<string, { tokens: number; calls: number; estimatedCost: number }> = {};
    ...
    return result as Record<ModelTier, { tokens: number; calls: number; estimatedCost: number }>;
  }
